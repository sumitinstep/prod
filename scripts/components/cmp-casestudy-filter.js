...
jQuery(document).ready(function($) {
	var componentId = '.cmp-casestudy-filter ';
	...
	if ( jQuery(componentId).length > 0 )
	{
		...
		jQuery(componentId+' #filter-product .checkbox, '+componentId+' #filter-industry .checkbox, '+componentId+'   #filter-region .checkbox, '+componentId+'  #filter-solution .checkbox, '+componentId+'  #filter-language .checkbox, '+componentId+'  #sortby').change(function(){	
			...
			copySelectedOptionFromDesktopToMobileCasestudy();
			copySelectedSortFromDesktopToMobileCasestudy();
			
			updateWebBrowserPathCasestudy();
			updateFilterSelectedNumberCasestudy();
			updateMobileFilterSelectedNumberCasestudy();
			
			var queryString = jQuery('form#filter-bar').serialize();
			updateResultAreaDesktopCasestudy('#desktop-result', queryString);
			updateResultAreaMobileCasestudy('#mobile-result', queryString, 1);
		});
		...
		jQuery(componentId+' #mobile-menu-bar .cmp-casestudy-filter__close').click(function(e){	
			e.preventDefault();
			jQuery(componentId+' #mobile-filter').removeClass('filter-showup');
			jQuery(componentId+' #mobile-filter').attr('style','');	
			jQuery(componentId+' #mobile-menu-bar').attr('style','');
		...
		jQuery(componentId+" #keyword").keyup(delay(function (e) {
			...
			{
				updateAutoCompleteDesktopCasestudy();
			}
		...
		jQuery(componentId+" #mobile-keyword").keyup(delay(function (e) {
			...
			{
				updateAutoCompleteMobileCasestudy();
			}
		...
		jQuery(componentId+' #mobile-filter #filter-option-done').click(function(event){
			...
			copySelectedOptionFromMobileToDesktopCasestudy();
			copySelectedSortFromMobileToDesktopCasestudy();
		...
		jQuery(componentId+' #mobile-reset-filter,'+componentId+' #desktop-reset-filter').click(function(event){
			...
			//updateSelectedMobileDropdownTextCasestudy();
			...
			updateResultAreaDesktopCasestudy(componentId+'  #desktop-result', queryString, 1);
			updateResultAreaMobileCasestudy(componentId+'  #mobile-result', queryString, 1);
		});
...
function updateFilterSelectedNumberCasestudy(){
	var componentId = '.cmp-casestudy-filter ';
...
function copySelectedOptionFromMobileToDesktopCasestudy(){
	var componentId = '.cmp-casestudy-filter ';
...
function copySelectedSortFromDesktopToMobileCasestudy(){
	var componentId = '.cmp-casestudy-filter ';
...
function copySelectedSortFromMobileToDesktopCasestudy(){
	var componentId = '.cmp-casestudy-filter ';
...
function copySelectedOptionFromDesktopToMobileCasestudy(){
	var componentId = '.cmp-casestudy-filter ';
...
function updateMobileFilterSelectedNumberCasestudy(){
...
function updateWebBrowserPathCasestudy(){
	...
	finalURL = getDropdownWebBrowserPathCasestudy(finalURL, componentId, 'filter-product-family', ' #filter-product ')
	finalURL = getDropdownWebBrowserPathCasestudy(finalURL, componentId, 'filter-industry', ' #filter-industry ')
	finalURL = getDropdownWebBrowserPathCasestudy(finalURL, componentId, 'filter-region', ' #filter-region ')
	finalURL = getDropdownWebBrowserPathCasestudy(finalURL, componentId, 'filter-solution', ' #filter-solution ')
...
function getDropdownWebBrowserPathCasestudy(finalURL, componentId, param, dropdownID)
{
	var n_items = jQuery(componentId+' '+dropdownID+' input.checkbox:checked').length;
	if (n_items > 0)
	{
...
{
	var selected_solution = jQuery('.cmp-casestudy-filter input#filter-solution').val();
	jQuery('.cmp-casestudy-filter .filter-solution').each(function(index, value) {		  
...
function updateResultAreaDesktopCasestudy(targetArea, queryString, paged)
{
	var componentId = '.cmp-casestudy-filter ';
	...
	ajaxUrl = 'http://localhost/meraki-prodtest/?rest_route=/'+postType+'/v1/getitems&'+queryString+'&page_number='+paged+'&range='+9;
	
	jQuery(componentId+' .desktop-result .loading').show();
	jQuery(componentId+' .desktop-result .loading__icon').show();

	clearNumberOfResultCaseStudy();
	...
	jQuery.ajax(ajaxUrl, {
		success: function(data) {
			...
			updateFilterSelectedCasestudy();
			
			updateNumberOfResultCasestudy(); 
		}, 
...
function updateResultAreaMobileCasestudy(targetArea, queryString, paged)
{
	var componentId = '.cmp-casestudy-filter ';
...
function updateAutoCompleteDesktopCasestudy()
{
	var componentId = '.cmp-casestudy-filter ';
...
function updateAutoCompleteMobileCasestudy()
{
...
function resetAllFilterCasestudy()
{
	var componentId = '.cmp-casestudy-filter ';
...
/*
function updateSelectedMobileDropdownTextCasestudy(){
...
function updateFilterSelectedCasestudy(){
...
function desktopLoadmoreCasestudy()
{
	var componentId = '.cmp-casestudy-filter ';
...
function mobileLoadmoreCasestudy() {
...
function casestudy_keyword_search(mobile){
